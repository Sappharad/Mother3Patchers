	...
	public class LibUPS
	{
		public static void ApplyUPS(string inputPath, string outputPath, string patchPath){
			...
			if (!File.Exists (patchPath)) {
                throw new FileNotFoundException("Arquivo de patch não encontrado!");
			}
			...
			if (patchFile.Length < 16) {
				...
				patchFile.Close ();
				outFile.Close ();
                throw new Exception("Arquivo de correção é muito pequeno!");
			}
			...
			if(crc_patch != CalculateFileCRC32(patchFile, patchFile.Length-4)){
				...
				patchFile.Close ();
				outFile.Close ();
                throw new Exception("Patch é corrupto!");
			}
			...
			if(cache[0] != 'U' && cache[1] != 'P' && cache[2] != 'S' && cache[3] != '1'){
				...
				patchFile.Close ();
				outFile.Close ();
                throw new InvalidDataException("Não é um arquivo UPS!");
			}
			...
			if(crc_in != CalculateFileCRC32(inFile, (long)size_input)){
				...
				patchFile.Close ();
				outFile.Close ();
                throw new Exception("ROM de entrada não é o arquivo correto!");
			}
			...
			if(crc_out != CalculateFileCRC32(outFile, (long)size_output)){
				...
				patchFile.Close ();
				outFile.Close ();
                throw new Exception("O arquivo de saída não está correto!");
			}
